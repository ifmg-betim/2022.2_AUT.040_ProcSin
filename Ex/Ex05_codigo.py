def capturaSinalSecreto(ar):
    ...
    X=requests.get('https://github.com/ifmg-betim/2022.2_AUT.040_ProcSin/blob/main/Ex/Ex05_arquivo.w123?raw=true')
    A("Ex05_arquivo.w123","wb").write(X.content)
    fs,v=wavfile.read('Ex05_arquivo.w123')
    v=v[:,0]; L=np.arange(s(v))/fs; np.random.seed(ar*3+4); c1,c2,c3=np.random.randint(0,11,3);
    ...
    d2,n1=signal.iirdesign([(c4-2000)/fs,(c4+2000)/fs],[(c4-800)/fs,(c4+800)/fs],1,40)
